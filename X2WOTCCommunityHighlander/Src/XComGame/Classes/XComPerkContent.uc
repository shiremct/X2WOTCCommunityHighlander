...
// *******************************
// Start Issue #142 (Part 1 of 1):
// Add new function that returns all, rather than the first, perk duration effect. Replace the variable handling and the function call in
// X2Action_AbilityPerkDurationEnd to call this function and store the result as an array. Old function is still active below, but no longer referenced. 

static function GetAssociatedDurationPerkInstances( out array<XComPerkContentInst> ValidPerks, XComUnitPawnNativeBase Caster, XComGameState_Effect EffectState, optional int HistoryIndex = -1 )
{
	local XComPerkContentInst PerkInst;
